...
// ─── Constants ────────────────────────────────────────────────────────────────
pub const DEFAULT_AUTHOR: &str = "Freedom Trails, LogicCuteGuy";
pub const DEFAULT_SOURCE: &str = "Freedom Trails";
...
const AI_KEYWORDS: &[&str] = &[
    "ChatGPT", "DALL-E", "DALL_E", "DallE", "OpenAI",
    "openai", "chatgpt", "dall-e", "dall_e", "dalle",
    "gpt-image", "gpt_image", "gptimage", "DALL·E", "dall·e",
    "Stable Diffusion", "stable-diffusion", "stable_diffusion",
    "stablediffusion", "Stability AI", "stability-ai",
    "StableDiffusion", "SDXL", "SD 1.5", "SD 2.1",
    "ComfyUI", "comfyui", "A1111", "AUTOMATIC1111",
    "Midjourney", "midjourney", "MIDJOURNEY",
    "Adobe Firefly", "firefly", "Firefly", "FIREFLY",
    "Adobe Sensei", "Adobe GenAI",
    "Imagen", "imagen", "Imagen 2", "Imagen 3",
    "Google AI", "Google GenAI", "Gemini",
    "Make-A-Scene", "Make-A-Video", "Emu", "Meta AI", "Llama Gen",
    "Bing Image Creator", "DALL-E 3", "Designer",
    "Microsoft Designer", "Copilot",
    "Amazon Titan", "Bedrock",
    "Leonardo AI", "leonardo.ai", "Leonardo",
    "Playground AI", "Craiyon", "NightCafe", "DreamStudio",
    "Runway", "Pika", "Sora",
    "通义万相", "Tongyi Wanxiang", "文心一格", "ERNIE ViLG",
    "LiblibAI", "海艺AI", "SeaArt", "像素蛋糕", "吐司AI",
    "6pen", "MewXAI", "Vega AI",
    "Ideogram", "Flux", "Kandinsky", "Disco Diffusion",
    "Wombo", "Lensa", "Remini", "Topaz",
    "Generative AI", "GenAI", "genai", "GENERATIVE",
    "AI Generated", "ai-generated", "ai_generated",
    "AI Art", "ai-art", "ai_art",
    "Machine Learning", "Neural Network", "Deep Learning",
    "Text-to-Image", "text-to-image", "txt2img",
    "trainedAlgorithmicMedia", "synthetic media",
    "c2pa", "C2PA", "content credentials",
    "ContentCredentials", "content-credentials",
    "Juggernaut", "DreamShaper", "Realistic Vision",
    "Deliberate", "Epic Diffusion", "OpenJourney",
    "Anything Diffusion", "Waifu Diffusion",
];
...
fn has_ai_keywords(data: &[u8]) -> bool {
    ...
    AI_KEYWORDS.iter().any(|kw| text_lower.contains(&kw.to_lowercase()))
}
...
fn png_read_chunks(data: &[u8]) -> Vec<PngChunk> {
    ...
    while offset + 8 <= data.len() {
        ...
        chunks.push(PngChunk { chunk_type: ctype, data: chunk_data });
        offset += 12 + length;
        if &ctype == b"IEND" {
            break;
        }
    ...
    if ct == "zTXt" {
        if let Some(ni) = cdata.iter().position(|&b| b == 0) {
            if ni + 2 < cdata.len() && cdata[ni+1] == 0 {
                ...
                if has_ai_keywords(kw.as_bytes()) { return true; }
                let raw = &cdata[ni+2..];
                let mut buf = Vec::new();
                let _ = flate2::read::ZlibDecoder::new(raw).read_to_end(&mut buf);
                if has_ai_keywords(&buf) { return true; }
            }
        }
        return false;
    }
...
fn png_custom_chunks(meta: &Metadata) -> Vec<Vec<u8>> {
    let mut chunks = Vec::new();
    let desc = if meta.description.is_empty() { format!("Created by {}", meta.author) } else { meta.description.clone() };
    let credit = if meta.credit.is_empty() { meta.author.clone() } else { meta.credit.clone() };
...
pub fn process_png(data: &[u8], meta: &Metadata) -> ProcessResult {
    ...
    ProcessResult { output: out, removed, file_type: "png".to_string() }
}
...
pub fn process_jpg(data: &[u8], _meta: &Metadata) -> ProcessResult {
    ...
    ProcessResult { output: out, removed, file_type: "jpg".to_string() }
}
...
pub fn process_webp(data: &[u8], _meta: &Metadata) -> ProcessResult {
    if data.len() < 12 || &data[..4] != b"RIFF" || &data[8..12] != b"WEBP" {
        return ProcessResult { output: data.to_vec(), removed: 0, file_type: "webp".to_string() };
    }
    let mut removed = 0;
    let mut out = Vec::with_capacity(data.len());
    ...
    while i + 8 <= data.len() {
        let cid = &data[i..i+4];
        let csize = u32::from_le_bytes([data[i+4], data[i+5], data[i+6], data[i+7]]) as usize;
    ...
    ProcessResult { output: out, removed, file_type: "webp".to_string() }
}
...
pub fn process_mp4(data: &[u8], _meta: &Metadata) -> ProcessResult {
    ...
    fn walk(data: &[u8], out: &mut Vec<u8>, removed: &mut usize, start: usize, end: usize) {
        let mut pos = start;
        while pos + 8 <= end {
            let sz = u32::from_be_bytes([data[pos], data[pos+1], data[pos+2], data[pos+3]]) as usize;
            let tp = if pos + 8 <= end { &data[pos+4..pos+8] } else { b"" };
            if sz < 8 || pos + sz > end {
                out.extend_from_slice(&data[pos..end]);
                return;
            }
            if (tp == b"udta" || tp == b"meta" || tp == b"ilst") && mp4_has_ai_in_range(data, pos, pos+sz) {
                *removed += 1;
            } else if tp == b"moov" || tp == b"trak" || tp == b"mdia" || tp == b"minf" || tp == b"stbl" {
    ...
    ProcessResult { output: out, removed, file_type: "mp4".to_string() }
}
...
pub fn process_pdf(data: &[u8], meta: &Metadata) -> ProcessResult {
    ...
    while let Some(start) = out[i..].windows(pattern.len()).position(|w| w == pattern) {
        ...
        if let Some(end_rel) = out[abs_start..].windows(close_pattern.len()).position(|w| w == close_pattern) {
    ...
    if let Some(eof_pos) = out.windows(5).rposition(|w| w == b"%%EOF") {
        ...
        if !meta.author.is_empty() { info.push_str(&format!("/Author ({})\n", meta.author)); }
        if !meta.source.is_empty() { info.push_str(&format!("/Creator ({})\n", meta.source)); }
        if !meta.title.is_empty() { info.push_str(&format!("/Title ({})\n", meta.title)); }
        if !meta.description.is_empty() { info.push_str(&format!("/Subject ({})\n", meta.description)); }
        if !meta.keywords.is_empty() { info.push_str(&format!("/Keywords ({})\n", meta.keywords)); }
        out.splice(eof_pos..eof_pos, info.bytes());
    }

    ProcessResult { output: out, removed, file_type: "pdf".to_string() }
}
...
pub fn process_ooxml(data: &[u8], meta: &Metadata) -> ProcessResult {
    ...
    let mut zin = match ZipArchive::new(cursor) {
        Ok(z) => z,
        Err(_) => return ProcessResult { output: data.to_vec(), removed: 0, file_type: "ooxml".to_string() },
    };
    ...
    let mut buf = Vec::new();
    {
        ...
        for i in 0..zin.len() {
            ...
            if fname == "docProps/core.xml" && has_ai_keywords(&content) {
                let creator = if meta.author.is_empty() { "".to_string() } else { meta.author.clone() };
                let title = if meta.title.is_empty() { "".to_string() } else { meta.title.clone() };
                let desc = if meta.description.is_empty() { "".to_string() } else { meta.description.clone() };
                let kw = if meta.keywords.is_empty() { "".to_string() } else { meta.keywords.clone() };
                let comments = if meta.comments.is_empty() { "".to_string() } else { meta.comments.clone() };
                content = format!(
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
  ...
  <cp:comments>{comments}</cp:comments>
                ...
                let app = if meta.source.is_empty() { "".to_string() } else { meta.source.clone() };
                let cat = if meta.category.is_empty() { "".to_string() } else { meta.category.clone() };
                content = format!(
                    r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties">
  ...
  <Category>{cat}</Category>
    ...
    ProcessResult { output: buf, removed, file_type: "ooxml".to_string() }
}
...
pub fn process_mp3(data: &[u8], _meta: &Metadata) -> ProcessResult {
    ...
    ProcessResult { output: out, removed, file_type: "mp3".to_string() }
}
...
pub fn process_flac(data: &[u8], _meta: &Metadata) -> ProcessResult {
    ...
    if data.len() < 4 || &data[..4] != b"fLaC" {
        return ProcessResult { output: data.to_vec(), removed: 0, file_type: "flac".to_string() };
    }
    ...
    ProcessResult { output: out, removed, file_type: "flac".to_string() }
}
...
pub fn process_exe(data: &[u8], _meta: &Metadata) -> ProcessResult {
    ...
    ProcessResult { output: out, removed, file_type: "exe".to_string() }
}
